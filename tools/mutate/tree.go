...
// something. Raising it is cheap; lowering it below what the suite honestly
// needs is the worst thing that can be done to this tool, because a test binary
// killed by the deadline exits non-zero exactly like a failing one — every
// mutation would report caught and the sweep would be testing nothing.
const suiteTimeout = "90s"

// locate is the file to break and the package to run it in.
//
// A path relative to the repository root, always: "server/keys.go". It used to
// take a bare name as meaning server/ and a slash as meaning the root, which
// saved some typing and cost a rule nobody could keep in their head — and a
// sweep that breaks a file nobody meant to break is the failure this whole tool
// exists to avoid.
